...
type: skill
title: "plan"
description: "このノードのゴール: spec.toml に従い、徹底的な実装計画を plan artifact（≤200 行）に"
tags: [skill, workflow]
---
...
`docs/CONTEXT-FORMAT.md`)にその場で追記(新規作成時は OKF frontmatter `type: glossary` を付与)。
**具体シナリオでエッジを突き** 概念の境界を強制的に明らかにする。述べた挙動は **実装と照合** し矛盾を表に出す。コードは編集しない。
...
2. **`plan` artifact（≤200 行）を書く** ── 最低限以下を含めること:
   ...
   - **consequential な設計決定の一覧（design-writing ①）** ── 後で変えると「その1箇所」を
     超えて波及する決定（API/シグネチャ・スキーマ/永続形式・エラー/失敗の契約・不変条件・
     モジュール分解＝責務の所在・AC が観測する挙動）。各々「どう機械検査に焼くか」（型/
     スキーマ/契約テスト）を併記。mechanical（1箇所で直る＝変数名/内部アルゴリズム/ログ）は
     「実装裁量」と明記して残す（過剰仕様＝設計のコード化を避ける）。下の「設計を閉じる」参照。
...
## 設計を手戻りが出ない形に閉じる（design-writing ①②）

目的は「正しい設計」でなく **手戻りを消す**こと。`手戻り ∝ (実装者の“不確定な自由度”) ×
(誤りの発覚の遅さ)`。**deep 変更時**（損失に近い/新規 Why/低検出 ── research の深度 triage が
deep だったとき）に行う。明確に trivial（cosmetic / 既存 ADR の範囲内）なら軽く飛ばしてよい。

### ① 自由度を閉じる ── 独立実装者ドッグフードで可視化
plan を**答え（想定実装）を伏せて**独立サブエージェント（worker とは別の実装者役）に渡し、こう指示する:
> 「これを新規実装する。コードを書く前に、**設計が決めていない所で推量する点・質問したい点を
> 全部出せ**。各々『間違えたら1箇所を超えて波及するか（caller/テスト/データを壊す・移行が要る）』を
> 述べよ。一般論でなくこのコードベースに即して。」

返った **assumption（黙って埋める所）＋question ＝閉じ損ねた自由度**。実装者が黙って踏む誤推量を
設計時に前出しできる（research の独立詰問者の双子・自己採点を避ける独立評価者の前倒し）。
- **consequential（波及する）→ plan で閉じる**（決定を明記＋機械検査への焼き方を併記）。
- **mechanical（1箇所で直る）→「実装裁量」と明記して残す**。
- 迷えば閉じる（非対称コスト）。consequential な assumption が**ゼロになるまで** loop。

### ② 早く安く捕まえる形で書く（fail-fast）
発覚段で手戻りが指数増（設計時 ≪ コンパイル/型 ≪ テスト ≪ 出荷後）。
- **falsifiable に**：「適切に処理」でなく**明示の不変条件/契約/「失敗Xを手段Yで防ぐ」**。
- **consequential 決定を機械検査媒体へ**：型/スキーマ/契約テストに焼く（不正状態を表現不能に＝
  誤実装がコンパイル/テストで落ちる）。散文に残すのは WHY・理由・通念からの逸脱だけ。
- **各 AC → test 導出可能**な粒度に。導出テストは下流の差分 mutation（test ノードの
  `mutation_diff`）で誤検出能力を確認する（②だけでは「できたつもり」が残る）。

### 限界（overclaim しない）
①②は**必要条件で十分でない**。閾値の正しさ等は①の詰問側の責務で、②の型は**一貫性は守るが
正しさは守らない**。残差は下流保険（mutation/テスト）が詰める。

### 記録（非ブロッキング ── gate ではない）
```
harness report-evidence design_closure '{
  "verdict":"closed",                 # または "trivial_skip"（trivial 時）
  "dogfood":"independent",
  "consequential_closed":["...（決定＋焼いた型/スキーマ）..."],
  "mechanical_left":["...（実装裁量に残した点）..."]
}'
```
効果測定（導入後に下流 mutation の missed / review 指摘が減るか）用の記録で、**通過をブロック
しない**（保証できない性質を強制する existence-only gate は支配的設計として採らない）。

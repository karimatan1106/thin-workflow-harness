...
1. **plan / spec / 失敗 test の確認**
   ...
   <test cmd>                         # どの test が fail しているか
   ```

   **consequential な設計制約を目の前に出してから書く（design-writing ③）** ──
   plan の `consequential な設計決定の一覧` と `design_closure` evidence を**今ここで読み直す**。
   research/plan で読んだ記憶に頼らない（実装者は設計が黙る/曖昧な所で既定パターンに drift する）。
   特に **通念から外れる決定（anti-prior）**を確認して実装する（例「retry 可能に見えるが retry
   するな＝二重実行」）。consequential 制約が型/スキーマに焼かれていれば、それを使う形で書けば
   コンパイル/テストが逸脱を弾く。焼かれていない制約があれば plan 不足として `harness back`。

2. **実装（最小増分・機械駆動 ── loop speed (a)）** ── plan の順序に従って**対象ファイルを直接
   編集する**（`edit-file` という harness コマンドは存在しない）。
   **検証できる最小増分しか書かない**（大きな塊を書いてから間違いに気づく＝最大の手戻り源）。
   1増分ごとに下の**内側ループ（高速・scope 検査）**を回し、**red→green が駆動**する（自己採点で
   「できた」としない）。**緑になったら止める**（無断 refactor / 最適化 / YAGNI 違反をしない）。
   各変更は spec の `requirement.files` の範囲内に閉じる。範囲外を触りたくなったら
   plan を見直し（`harness back "plan に <file> が無い: ..."`）。

3. **内側ループ＝blast radius に scope した高速検査（loop speed (b)）** ── 各増分後に**最短で
   ground-truth を返す**。`--workspace` / フルビルドを内側で回さない（毎増分フルは致命的に遅い）:
   ```
   <fast check cmd>     # blast radius のモジュール/パッケージだけ・dev profile（秒オーダ）
   <focused test cmd>   # 今 green にしたい failing test だけ
   ```
   scope は spec `requirement.files` から特定する。設計の consequential 制約が型/スキーマに焼かれて
   いれば（design-writing ②）、この高速 check が逸脱を即弾く。

4. **外側ゲート＝遷移前に1回だけ full（速さでなく徹底）** ── 内側ループで緑になったら、ノードを
   進める直前に全体を通す（cross-scope の退行はここで捕まえる）:
   ```
   <full build cmd>    # mandatory_gates の cmd_exit_0 (check/build)
   <full test cmd>     # exit_gate cmd_exit_0 <test>
   ```

...
| セキュリティレビュー | `/security-review` skill（多段解析）— security ノード（`skills/06-security.md`）が invoke | security ノードの skill（`06-security.md`）が `/security-review` の手順を移植した helper（`skills/security-review.md`）を取り込む ＋ `cmd_exit_0` スキャナ |
| コードレビュー | `/review` skill — review ノード（`skills/07-review.md`）が invoke | review ノードの skill（`07-review.md`）が `/review` の手順を移植した helper（`skills/code-review.md`）を取り込む ＋ spec チェックリスト自己レビュー |
...
- `skills/` には各能力の*ポータブルな*版（手順）が入る — Phase 1（Claude Code 無し）に substance を提供。ノードの skill は常に*手順*を持ち、文面に「host に対応する組込み（plan モード / `/security-review` / `/review` / AskUserQuestion 等）があればそれを優先して使い、無ければ以下の手順に従え」と書く。
...
- ノードの skill は常に*手順*を持つ。host に対応する組込み機能（plan モード / `/security-review` / `/review` / AskUserQuestion 等）があればそれを優先して使い、無ければ skill 内の手順に従う、と skill 文面に書く。`[meta].host` の値は「どの組込みが使えるか」のヒントとして skill 文面の判断に使われる（テンプレート展開はしない）。

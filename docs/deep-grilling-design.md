...
## 9. preservation トラックへの拡張（preservation 専用レンズ・2026-06-30）

§4 の3レンズ（STPA損失 / 機能正しさ / Ousterhout）は **forward 開発向き**で、preservation（挙動保存=rehost/migration、oracle が逆＝旧システムの実挙動）トラックの支配的失敗 ＝ **omission** を直接生成しない。preservation の research（`skill_templates/p01-research.md`）は当初、固定カタログ（quirk クラス A〜H）＋人間承認のみで **grilling ループが不在**だった一方、`preservation_workflow.toml` は「入力空間列挙の完全性は research の deep-grilling の責務」と**委譲先を空参照**していた。これを埋める。

**preservation 専用3レンズ**（骨格＝独立詰問者・loop-until-dry・OR トリガーは §5/Phase2 を流用。A〜H は「答え」から **除去不能 seed** へ降格）：
- **レンズ1 捕獲漏れ（omission）**：`I/O境界 × HAZOPガイドワード{未知入力/稀パス/時間依存(年次・うるう・和暦・TZ)/状態依存(初期DB状態・upsert衝突)/環境入力(clock・locale・charset)}`。**加えて全出力フィールドを raw passthrough か 派生（計算/分類）かで仕分け**、派生は rewrite で計算ロジックごと黙って脱落し既定/空/NULL に落ちる最大の omission リスクとして「旧の計算規則・曖昧時の挙動・分類不能時の literal 値」を問う（旧が計算していた列を新が NULL 化してもエラーは出ない）。
- **レンズ2 オラクル忠実性**：rewrite が保存でなく「直す/正規化する」誘惑はどこか。**期待値は旧の捕獲出力由来か合成（synthetic）か** ── 合成 fixture は本番 NULL/空でも pass し omission を隠す（captured-only 強制・各規則に witness 隣接バグ）。
- **レンズ3 等価とフィルタ波及（両側の誤り）**：各等価規則の (a)本物の divergence を masking しないか (b)必然差で signal を埋もれさせないか。**最重要＝どの下流 consumer がどの列で filter/join/key するか**：その列が黙って既定/空/NULL になると consumer の挙動が**エラー無しで**変わる。各 partition に「沈黙の波及先」を併記。

**ブラインド A/B 検証（2026-06-30）**：題材＝market_type 分類が TS→Rust 移行で移植脱落（全件NULL→spot delist 誤表示・回帰testは合成値で本番NULL未再現）を伏せ、(A)新レンズ vs (B)A〜H カタログのみ を同一 sanitized ブリーフ（market_type を1列として混ぜ telegraph せず・repo読取禁止）に当てた。**両者とも market_type を載せたが、差は hit/miss でなく framing と構造的信頼性**：A は「派生列の計算ロジック脱落→NULL→下流フィルタ無言破綻」を構造的（全列→派生か?）に最優先で出し、**合成 fixture の罠（実バグの第二の真因）も独立検出**。B は class E（未初期化メモリ）を「NULL/既定値の差」へ generous に拡大解釈して初めて触れ（素直に適用すれば落ちる）、因果は逆向き（「TS が NULL を保て」＝実際は TS が計算）、合成 fixture 罠は未検出。**結論＝勝ち筋は framing 正しさ × 構造的信頼性 × カタログが生成しない meta-risk**（§3 Phase3「専用カタログ却下＝project 固有 tail を取りこぼす」と一致。ここでの tail＝派生列ロジック脱落は A〜H のどのクラスでもない）。

**実装と確認**：scaffold `skill_templates/p01-research.md` に4編集（詰問セクション新設・partition を レンズ1 生成＋`field/derivation/downstream_filter`・等価規則を両側検査＋captured-only・禁止3項）。`cargo install`（exit0）→`--template preservation` 再生成で新文言反映・`harness validate 5 ノード` 破損なし → **出荷 skill 本文 as-built だけで再検出する成果物テスト合格**。新 gate primitive ゼロ。**限界**＝§7/§8 と同じ：詰問は omission を減らすが完全性は証明しない。最終歯止めは下流（differential/coverage/飽和/独立評価者）。

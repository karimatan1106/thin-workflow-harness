...
- `harness record-artifact plan ...` → exit gates: `plan_artifact_exists` ✓ `max_lines plan 200` ✓ **`json_has plan_approval verdict eq approved`**（`harness ask`「この plan で進める? [承認/修正]」— 人間チェックポイント2つ目）✓ → advance。
...
- exit gates: `impl_artifacts_exist` ✓ `impl_artifacts_size_ok`（`--tag new` は ≤200）✓ `no_regex`（禁止語）✓ harness が**テストコマンドを再実行**して exit 0 確認 ✓ `cmd_exit_0 "cargo check"`（**workspace 全体** — domain をまたぐ署名 break をここで捕まえる、弱いリンク #1 の安い proxy）✓ → advance。
...
3 ブランチ worktree を `git merge --no-ff`（逐次なら no-op）→ 結合テスト `cmd_exit_0 "cargo nextest run -p identity"` ＋ **UI も触ったので `cmd_exit_0 "pnpm --filter identity-ui test"`**（← test/join ノードの gate は blast radius の言語/パッケージから導出すべき、弱いリンク #5）＋ `cmd_exit_0 "cargo check"` → exit gates: 結合 `cmd_exit_0` ✓ `traceability_closed` ✓ `count_non_decreasing` ✓ → advance。
...
- advance → F-2FA.5（`enforce.rs` 変更、.3 の `verify_totp` 依存、機械的）→ advance。
